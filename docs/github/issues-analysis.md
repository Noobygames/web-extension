...
The core question — *does an issue have a PR, and does that PR fix it?* — has a blunt answer here:
...
*bug · opened 2026-08-03 by GeGeGM · no linked PR*

**Status: CONFIRMED.** The reported code is unchanged.

`getLocalStorageSize()` at `src/ogkush.js:14651` iterates `window.localStorage` only:

```js
for (var x in localStorage) {
  var amount = localStorage[x].length / 1024 / 1024;
  if (x == "ogk-data") { ogi += amount; } else { other += amount; }
}
```

The result is rendered as `${size.total} / 5 Mb` at `src/ogkush.js:14994`. Everything the content script stores — the per-universe `DataHelper` blob, `ogi-scanned-<universe>`, and the `<universe>-<key>-information` keys written by `src/ctxcontent/services/universe.storage.js` — lives in `chrome.storage.local` and is invisible to this counter.

**Not mentioned in the issue, and arguably worse than the wrong label:** the same number drives an automatic purge. `src/ogkush.js:1691-1693`:
...
So the threshold that triggers data deletion is computed from an incomplete measurement, and `purgeLocalStorage()` (`src/ogkush.js:14671`) likewise only clears `localStorage`. A user whose real footprint is dominated by `chrome.storage.local` never trips the purge; a user with unrelated page-local keys trips it early.
...
*opened 2025-04-09 by Nightmaster · no linked PR · no comments*
...
- `src/ogkush.js:13322-13333` — `selectShips()` calls `fleetDispatcher.selectShip(...)` then `fleetDispatcher.refresh()`.
- `src/ogkush.js:13335-13345` — `preselectShips()` loops `shipsOnPlanet` and calls `selectShips()` **and** `fleetDispatcher.refresh()` again per ship, so a planet with N ship types triggers up to 2N refreshes back to back.
- `preselectShips()` runs from `start()` (`src/ogkush.js:1653`), i.e. while OGame's own async `fetchTargetPlayerData` may still be in flight.

`fleetDispatcher` is referenced ~370 times across `ogkush.js`, so an exhaustive audit is out of scope here.
...
*opened 2022-12-05 by 4x10m · linked PR #132 (**closed, not merged**)*
...
3. The content-script listener guards on the wrong condition — `src/ctxcontent/index.js:59-63`:
   ...
   ```
   `waitFor(() => dataHelper)` only waits for the *object reference* to exist. In `processData()` (`src/ctxcontent/index.js:26-47`) `dataHelper = universes[UNIVERSE]` is assigned synchronously right after `update()` is *called*, not after it resolves. So `dataHelper` becomes truthy while `loading` is still `true`.
...
*opened 2021-07-21 by Efraid · no linked PR*
...
The production code has been substantially rewritten since: `src/ogkush.js` now folds in `json.lifeformBonus.productionBonus` and per-planet `lifeformPlanetBonus[...].productionBonus` in every production path (e.g. `src/ogkush.js:1990`, `:2033`, `:2313`, `:2493`, `:2528`), plus a class/officer/item bonus layer that did not exist in 2021.
...
*bug · opened 2021-01-06 by alexisfasquel · no linked PR*
...
*bug · opened 2021-01-05 by rteuwens · linked PR #33 (**closed, not merged**)*
...
Current code does the lookup the issue asked for. `src/ogkush.js:168` defines `UNIVERSVIEW_LANGS` (`en, cs, es, fr, de, da, hr, it, hu, nl, pl, pt, ro, ru, sk, sv, tr, …`), and `src/ogkush.js:1548-1552` resolves the segment from the page's declared game language with an `en` fallback:
...
`OgamePageData.gameLang` reads `<meta name="ogame-language">` (`src/util/OgamePageData.js:4`), which is `en` on `.us` servers — exactly the mapping the issue requested. It is consumed at `src/ogkush.js:4652` (TrashSim), `:4673` (Ogotcha), `:3511` and `:14229`.

PR #33 ("Temp fix") was closed unmerged in 2021; the fix landed independently.

**Caveat:** christiankm's follow-up in the same thread is a *different* symptom (only one TrashSim button on Edge/Windows, invalid URL). That was never confirmed and involves a suspected extension conflict. If it still matters it deserves its own issue rather than keeping #4 open.
...
*feature request, improvements · opened 2023-11-26 by GeGeGM · no linked PR*
...
*opened 2022-12-31 by KongGal · no linked PR*

**Status: NOT IMPLEMENTED as requested; the follow-up comment is implemented.**

Requested: divide available cargo ships by the number of *free expedition slots*, so repeated clicks send evenly sized fleets.

Current behaviour (`src/ogkush.js:11470-11505`) sizes a **single** expedition: it computes the minimum cargo ships needed to reach `maxExpeditionPoints`, then tops up to `maxResources * options.expedition.limitCargo`, falls back to the other cargo type when one runs short, and can apply a fleet template. Nothing in that path reads the number of free expedition slots, so ship count per send does not scale with how many slots remain.

morganpizzini's 2024 comment ("let me choose which cargo type") **is** covered: `options.expedition.cargoShip` selects 202 vs 203 and is honoured at `src/ogkush.js:11485-11487`.
...
*opened 2022-11-04 by adaamz · no linked PR*
...
*feature request · opened 2021-01-11 by F1S1C0 · no linked PR*

**Status: NOT IMPLEMENTED.** The only phalanx-related code is a refresh-link click at `src/ogkush.js:13910-13911` and the building name in the tech table / translations. No range calculation, no display in the building detail view.
...
*feature request · opened 2021-01-11 by PlumbLeech · no linked PR*
...
*feature request · opened 2021-01-05 by christiankm · no linked PR*
...
- `options.rvalLimit` — minimum target rentability, editable in settings (`src/ogkush.js:14872` renders the input, `:15341` writes it back), default `1e6` (`src/ctxpage/conf-options.js:60`), consumed throughout the spy table (`SpyMessagesAnalyzer.js:437`, `:453`, `:636`, `:716`) and in `ogkush.js:13232`/`:13252`. Translations exist ("Minimal target rentability to be considered as interesting", `src/util/translate.js:1390`).
...
*feature request, improvements · opened 2021-01-05 by christiankm · no linked PR*
...
Loosely related: `ogkush.js:5512`/`:5517` already exempt `favorited` items from the 5-day auto-cleanup, so the "favorites are protected" principle exists elsewhere in the codebase.
...
kursion's 2022 comment points at the pin icon + sideStalk panel, which covers "follow this player" and is live (`src/util/stalk.js`, `OGIData.playerMarkers`).

The literal request — adding a player so **all** their planets enter the target list, and stay in sync as they colonise or move — is not implemented. `options.targetList` and the target list UI (`src/ogkush.js:3952-3955`, `:4696-4723`) are keyed by `data-coords`, i.e. per planet, with no player-level entity and no re-sync on planet changes.
...
*opened 2023-03-30 by awarkentin88 · no linked PR*
...
| Guide for running the extension locally | **Now answered.** `Makefile` + `scripts/build-unpacked.mjs` produce a loadable build (`make dev`, `make brave`); see the *Local development* section of `readme.md`. This was the concrete blocker for the would-be contributor in 2023. |
