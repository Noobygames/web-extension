...
Lint note: `npm run check` is **green and gating**. It runs in CI (`.github/workflows/test.yml`, after the tests) and covers `src/`, `test/`, `scripts/`, `config/` and `.eslintrc.cjs` itself; `.eslintignore` keeps vendored `src/libs/` out. `no-undef` is **on and useful**: OGame's own page globals are declared in `config/ogame-globals.cjs`, one shared list that `.eslintrc.cjs` and `test/src-references.test.js` both read — before it existed those 55 names produced 678 `no-undef` errors and hid two real ReferenceErrors. A new bare global therefore fails lint by design; `make globals` lists what `src/` reads, and `docs/ogame-globals.md` has the console snippet for checking a name against a live page before you add it. `.eslintrc.cjs` extends `prettier` and must **not** re-enable any stylistic rule (`indent`, `quotes`, `semi`, `linebreak-style`): it used to, those rules fought `prettier/prettier` over the same code, and the resulting 240 phantom errors kept lint permanently red — which is why six real findings sat unnoticed. Line endings are enforced by `.gitattributes` (`eol=lf`) plus Prettier `endOfLine: "lf"`, not by ESLint. Still do NOT run `npm run format` across the tree to clear an unrelated red: a whole-tree reformat as a side effect buries the real diff.

`packaging.sh` is bash + `sed -i` (GNU) + node — on Windows run from Git Bash (`make build` shells out to `bash`). It has `set -euo pipefail`: it used to run past a failed step and still exit 0, because bash reports the last command’s status. Zipping is `scripts/zip.mjs`, not the `zip` binary (deterministic, and Git Bash has no `zip`). Its last step is `scripts/verify-package.mjs`, which opens each zip — version stamped, no `manifest-firefox.json` left in, every manifest icon present, `chrome-extension://` only in the Chromium builds and `moz-extension://` only in the Firefox one, no `update_url` in the Edge one. Two of those checks exist because the bugs shipped: the Firefox CSS rewrite was `s/chrome/moz/g` over the whole file, and the Edge `update_url` removal was `sed -i '31d'` whose line number had drifted onto an icon.

CI: `.github/workflows/test.yml` (tests + lint), `build.yml` (all three zips on every push/PR, verified, uploaded as artifacts, `web-ext lint` on Firefox), `main.yml` → Chrome/Edge via BPP and `deploy_amo.yml` → Firefox via `web-ext`, the last two `workflow_dispatch` with a version input.

**`src/` cannot load as unpacked extension directly** — version placeholders (`0.0.0` in manifest, `__VERSION__` in `src/platform/version.js`) need substitution first. Use `make dev` / `make dev-firefox` (`scripts/build-unpacked.mjs`): same stamping + Firefox CSS rewrite as `packaging.sh`, but leaves loadable directory not zip, and strips two store-only manifest keys — `update_url`, and `extension_ids` whitelist inside `web_accessible_resources` (local build has different extension id, whitelist would lock build out). After editing `src/`, re-run `make dev` and reload extension.

`scripts/launch-brave.mjs` (behind `make brave` / `make brave-main` / `make brave-open`) autodetects Brave binary — override with `BRAVE_PATH` — and defaults to throwaway `--user-data-dir` under `dist/.brave-dev-profile`, because already-running Brave would just open tab and drop `--load-extension`. Also passes `--disable-features=DisableLoadExtensionCommandLineSwitch` for Chromium 137+ and `--disable-extensions-except=<build>`; latter because Web Store OGBI build would otherwise inject `ogCore.js` into same page and share `localStorage["ogk-data"]` key with local build. Both session-only flags, nothing uninstalled. `--keep-extensions` opts out, `--dry-run` prints command line without spawning.
...
| ----- | ----------------------------------------------------------- | --------------------------------------------------------------- |
| Entry | `src/main.js` → dynamic import of `src/ctxcontent/index.js` | `src/ogCore.js` (injected as `type="module"`)                   |
| Dirs  | `src/ctxcontent/**`                                         | `src/ogCore.js`, `src/ctxpage/**`                               |
| Has   | `chrome.*` APIs, `chrome.storage.local`, cross-origin fetch | OGame's DOM and page globals (`DOMPurify`, `LZString`, `Chart`) |

Boot chain: `main.js` (content script, `document_start`) does **not** wait for `DOMContentLoaded`. It applies the cached wide-layout classes to `<html>`, mints the bridge token and publishes it on `<html>`, then fires everything off at once: `injectScript()` for `libs/purify.min.js` and `ogCore.js`, plus a dynamic import of `ctxcontent/index.js` whose `main(callbackToken)` registers the callbacks and loads/refreshes the universe `DataHelper`. Nothing waits on anything else. `libs/lz-string.min.js` and `libs/chart.min.js` are **not** on the boot path: both are injected on demand over the `ogi-lzstring` / `ogi-chart` events, from `ensureLZString()` and the chart code respectively. `ogCore.js` awaits DOMContentLoaded itself (`domReady()` in the bottom IIFE) before touching DOM. Three consequences worth knowing:
...
- **The bridge token is minted in `main.js`, not in `platform/bridge.js`.** `pageContextInit()` throws without a token on `<html>`, which used to force `ogCore.js` to be injected only after the content module had loaded. `main.js` now creates it (`createCallbackToken()` has a hand-copied twin there — classic content script, cannot import) and passes it into `contentContextInit(map, token)`, so the 1.1 MB page bundle and the content bundle download side by side. Either half may initialise first; `pageContextInit()` still replaces the published token with `"1"`, and the content half no longer reads the dataset when it was handed a token.
- **Bundling is part of the build, and it splits.** `make dev` / `make build` run `scripts/bundle.mjs` (rollup, `treeshake: false`, no minification): `ogCore.js` and `ctxcontent/index.js` each collapse to one entry file, and **anything reached through a dynamic `import()` in `src/` becomes its own chunk under `chunks/`** — 502 KB entry, 25 chunks, of which a given page load fetches none to two. There is no chunk list in the script; rollup reads it off the import graph, and `chunks/*` is one `web_accessible_resources` entry in **both** manifests, so a new chunk needs no manifest edit. Load one through `loadChunk(name, () => import("..."))` (`src/platform/loadChunk.js`): it logs the name and resolves to `undefined` rather than letting a failed fetch reject into nothing. The specifier must be a **literal** — a path built from a variable splits into no chunk at all and 404s at runtime. `OGBI_NO_BUNDLE=1 make dev` skips bundling. `test/bundle.test.js` builds the real bundles, evaluates the page entry _and every chunk_, and caps the entry at 512 000 bytes; a bundler breaks module evaluation order silently and no other test would notice. **Treat that budget as spent.** The entry once crept to 574 bytes under the cap, so any static import added to a module the boot path already reaches failed the test; the keyboard shortcuts moved behind a dynamic import to buy it back. Do the same rather than raise the number - `ctxpage/stalk/*` (43.5 KB), `ctxpage/chat/index.js` (18.5 KB) and `game/itemImageID.js` (10.9 KB) are all still in the entry.
...
Startup profiler: `src/platform/perf.js`. Off and free by default; `localStorage.setItem("ogi-perf", "1")` (or `&ogi-perf=1` in the URL) makes `ogCore.js` print a console table of every boot phase, every `start()` step over 0.5 ms, and the `ogk-data` parse/write totals. See `docs/performance.md`.

**Plan marks are split across the budget line.** `ctxpage/planHighlight/plans.js` is entry-side and reads `OGBIData.upgradePlans` raw — it must never import `store/upgradePlans.js`, which drags the 93 KB of cost tables in. `ogCore.js` only fetches the drawing half (`planHighlight/index.js`, a chunk) when `hasPlansOnCurrentSide()` says yes. Categories come from `buildPageOf()` in `game/technoIds.js` (id ranges, cheap), not `categoryOf()` in `game/upgradeCost.js` (tables); `test/util/upgradeCost.test.js` asserts the two agree on every id in both tables.

**No `src/util/`.** Phase B of `refactoring-new.md` dissolved it into seven folders sorted by what the code is about, not by "didn't fit elsewhere": `game/` (OGame's rules — costs, production, roi), `ogame/` (reading the running page — `OgamePageData`, `pageContext.js`), `store/` (`OGBIData.js`, the `ogk-data` singleton), `ui/` (DOM building, popups, tooltips), `format/` (numbers, dates, the `Translator`/`i18n` tables), `platform/` (extension infrastructure — logger, the content/page bridge, `wait.js`; must not assume `chrome.*` exists, and imports nothing outside itself), `integrations/` (PTRE, Pantry, MMORPG-Stats). All seven are shared by both contexts except where a file's own imports say otherwise (only `platform/` is checked for that — `test/architecture.test.js`). `src/platform/runContext.js` (`isPluginContext()`, `injectScript()`) is the context-detection guard.

**Aborting requests.** Every `fetch` the extension makes takes `pageSignal()` from `src/platform/abort.js` — one controller per page load, aborted on `pagehide`. Never write `window.onbeforeunload = () => controller.abort()`: that property is a single slot, six modules were assigning it, and each assignment disarmed the previous one. A test in `test/util/abort.test.js` fails if the pattern comes back. Because OGame navigates on every view change, aborts are the normal case, not a failure: `isAbortError()` / `ignoreAbort()` for local handling, and `suppressAbortRejections()` (called once per context) drops the `Uncaught (in promise) AbortError` noise — but only while the page is actually leaving, so a real error still shows.

### Crossing the boundary

Preferred: typed bridge in `src/platform/bridge.js`. Content script calls `contentContextInit({command: {action: fn}})` (see `ctxcontent/index.js`); page calls `pageContextInit()` once then `await pageContextRequest(command, action, ...args)`. Commands today: `ptre.galaxy`, `messages.expeditionType`. Promise **rejects** (with `ResponseCallbackEvent`) when `success === false` — always `try/catch`. Docs: `docs/service.callbackEvent.md`, `docs/context.content.commands.md`.
...
- Page context: `src/store/OGBIData.js` — singleton over `localStorage["ogk-data"]`, and since Phase 4 of `refactoring.md` the **only** way in. Every setter writes through immediately, so `OGBIData.options = {...}` persists but `OGBIData.options.foo = 1` does **not** (mutating returned object skips setter; reassign, or call `OGBIData.Save()`). The write-through is deliberate and load-bearing — see `docs/performance.md` for why deferring it was tried and reverted.
  - **One logical change goes through its setter; a batch of related changes mutates `OGBIData.json.*` and ends in exactly one `OGBIData.Save()`. Never both** — a `Save()` behind a setter serializes the whole blob twice for one change, and `test/util/store-access.test.js` fails on it. `OGBeyondInfinity.init()` is the documented exception: ~60 defaults seeded as one batch, persisted by the single `Save()` in `start()`.
  - There is no `this.json` and no `saveData()` method. Both existed until Phase 4 (`init()` aliased `this.json = OGBIData.json`, and every mutation needed a manual `saveData()` behind it); the same static test fails if either comes back. `src/ctxcontent/data-helper.js` keeps its own unrelated `saveData()` for `chrome.storage.local`.
  - `OGBIData.upgradePlans` holds the player's planned upgrades (technology, from level, to level, per planet and moon); `OGBIData.needs` is the priced total of them plus the orders already submitted in game (`submittedOrders()`: OGame charges a build order when it _starts_, so the one running is free and the four queued behind it are not). Pricing needs `gameFormulas.js` plus both cost tables, ~93 KB the entry cannot carry, so `store/upgradePlans.js` and everything reaching it are **chunk-only** and `ctxpage/upgradePlans/sync.js` refreshes the cache. Nothing the planet bar statically imports may reach that store.
...
`src/ogCore.js` is **no longer the monolith** — Phase 3 of `refactoring.md` took it from 19k lines to ~1.8k. What is left is the boot sequence: `class OGBInfinity` with `constructor`/`init`/`start`, the ~40 methods that are still page wiring, and the bottom IIFE that constructs `OGBInfinity` and `Messages`, waits for `DOMPurify` and calls `ogCore.start()`. Everything else lives under `src/ctxpage/**` (stats, fleetdispatch, galaxy, planetbar, empire, empireOverview, settings, technoDetail, keyboard, eventbox, stalk, pantry, pageTweaks, upgradePlans, planHighlight) and the seven domain folders that replaced `src/util/**` (`game/`, `ogame/`, `store/`, `ui/`, `format/`, `platform/`, `integrations/` — see above). New features go there and get wired in — never appended to `ogCore.js`.

**How a page module gets its state.** An extracted module never receives the `OGBInfinity` instance; it takes a plain `context` object built by one of the controller's `…Context()` methods (`fleetContext()`, `galaxyContext()`, `empireContext()`, …). A `context` that carries a bound method or the instance itself defeats the whole cut, and `test/ogCore.construction.test.js` fails on it. Where a module genuinely has to hand work back — the loading gate around `updateInfo()`, `flyingFleet()`, the `keyboardActionSkip` setter — the context carries a callback, not a method reference.

**Four traps the extraction hit, all silent.** `test/ctxpage/module-wiring.test.js` and `test/ogCore.wiring.test.js` exist because none of these fail a build, a lint or a bundle:
...
3. A constant left behind in `ogCore.js` is a `ReferenceError` on first use.
4. A module nobody imports simply never runs; the tell is the bundle getting smaller.

**Testing `ogCore.js`.** The module exports `OGBInfinity` **for tests only** — nothing imports it at runtime, and `test/bundle.test.js` asserts it stays the only export of the page bundle. `test/ogCore.construction.test.js` builds it from a page fixture and checks the contexts it hands out; importing the module runs its boot IIFE, so that file's `setupBrowser()` URL uses `component=intro`, one of the three pages the IIFE bails out on before touching the DOM or the network. The constructor's DOM reads live in `src/ogame/pageContext.js` (`readPageContext` / `stripCoordinateBrackets`, 100% covered) — that seam is what makes `new OGBInfinity()` possible in a test at all. Page fixtures: `test/fixtures/ogamePage.js`.

**Message analyzers.** `src/ctxpage/messages/index.js` (`class Messages`) observes messages component, dispatches to array of analyzers. Each analyzer implements informal interface `support(tabId) → bool`, `clean(force)`, `analyze(messageCallable, tabId)`. Tab ids come from frozen `messagesTabs` map in that file. Analyzer implementations live under `src/ctxpage/messages/analyzer/`, moved there in Phase 6 of `refactoring.md` from `src/ctxcontent/services/analyzer/` — that directory name read as content-context, the one place `chrome.*` is safe to use, and these classes run in **page** context and never see it; new analyzer = drop class there, push into `#analyzers`.

`src/ctxpage/messages-analyzer/index.js` is the older path, still **live**: `ogCore.js`'s `messagesAnalyzer()` checks `this.page !== "messages"` itself and, on that page only, loads the module as a chunk and calls its default export with `.call(this)` — so this file still runs with `OGBInfinity` as its `this`, unlike every module Phase 3 extracted. It is **scheduled for deletion** — see the header comment in that file for the evidence and the one thing that has to move first (the `.msg_date` timezone rewrite, which no analyzer class covers). Until then, note that both paths accumulate into the same `OGBIData.expeditionSums` / `OGBIData.combats` keys and disagree on the shape of `harvest` (`[0,0]` here vs `[0,0,0]` in `HarvestMessagesAnalyzer`). Do not add features to the old path, and do not delete it before its tests exist.

**Two DOM builders, on purpose.** `ui/dom.js` exports `createDOM` (content set as `textContent`, adds `dropdownInitialized` to `<select>`) and `createDOMSanitized` (content set as `innerHTML` through `DOMPurify`, renders a numeric `0`, no select marker). The second was `OGBInfinity.createDOM()` until the Phase 1 cut and has ~52 call sites. They are not interchangeable: swapping one for the other either escapes markup that was meant to render, or renders markup that was meant to be escaped. `test/util/dom-and-wait.test.js` pins all three differences.

**OGame version: 13 and later only.** v12 support was dropped in Phase 2 of `refactoring.md`. All 34 `OgamePageData.isAtLeast_13_0_0` forks are collapsed to their v13 branch — write the v13 selector and nothing else; do NOT reintroduce a second branch. `src/ogame/pageData.js` is still the singleton over `<meta name="ogame-version">` (`gameLang`, `playerLang`, coordinates, …), and `isAtLeast_13_0_0` survives with exactly one caller: a boot-path log line saying OGBI will not work if the server reports < 13.0.0. Without the v12 branches the extension would otherwise just find nothing and look broken for no stated reason.

**Translation** goes through the `Translator` singleton in `src/format/i18n/translate.js` (static tables + runtime scraping of in-game names cached in `OGBIData.json.translations`). Static tables live one per language in `src/format/i18n/translations/<lang>.js` (de, en, es, fr, tr, br), each a frozen `{tech, res, text}` object. **English is statically imported** — it is the per-key fallback, so it has to be there before the first lookup; the other five hang off a literal `import()` each and become chunks, which is 67 KB the average player never fetches. `Translator.load()` pulls the player's table and must run **after `DOMContentLoaded`** (the language comes from a `<meta>` tag); `ogCore.js` starts it right after `domReady()` and awaits it before `start()`, so nothing between those two points may translate. Until it resolves, `translate()` answers in English rather than throwing.

Adding a string means editing **all six** files. Forgetting five is invisible at runtime — they fall back to English — so `test/util/translations.test.js` names the file and the key instead: same key set as `en` in every language, no empty strings (an empty string wins against the fallback and shows the player a blank label), all six frozen. There is no generator script and no `make translations` target.
