...
- Page context: `src/util/OGBIData.js` — singleton over `localStorage["ogk-data"]`. Every setter writes through immediately, so `OGBIData.options = {...}` persists but `OGBIData.options.foo = 1` does **not** (mutating returned object skips setter; reassign, or call `OGBIData.Save()`). The write-through is deliberate and load-bearing — see `docs/performance.md` for why deferring it was tried and reverted.
