...
- **Bundling is part of the build.** `make dev` / `make build` run `scripts/bundle.mjs` (rollup, `treeshake: false`, no minification) which collapses `ogCore.js` (69 modules, 1.15 MB, 7 levels deep) and `ctxcontent/index.js` into one file each. `OGBI_NO_BUNDLE=1 make dev` skips it. `test/bundle.test.js` builds the real bundles and evaluates the page one, because a bundler breaks module evaluation order silently and no other test would notice.
