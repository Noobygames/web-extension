...
function expeditionStats() {
  ...
  tabNames[LocalizationStrings.timeunits.short.day] = () => {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      let sums = OGBIData.json.expeditionSums[dateStr] || sum;
    ...
    let details = renderDetails(computeRangeSums(OGBIData.json.expeditionSums, new Date(), new Date()), () =>
      refresh()
    );
    div.appendChild(
  ...
  tabNames[LocalizationStrings.timeunits.short.week] = () => {
    ...
    for (let i = 0; i < 12; i++) {
      let range = computeRangeSums(OGBIData.json.expeditionSums, start, prevMonday);
      weeks.push(range);
  ...
  tabNames[LocalizationStrings.timeunits.short.month] = () => {
    ...
    let totals = [];
    for (let i = 0; i < 12; i++) {
      let range = computeRangeSums(OGBIData.json.expeditionSums, lastDay, firstDay);
      months.push(range);
      let total = getTotal(range);
      totals.push({
        profit: total,
