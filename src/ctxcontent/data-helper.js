...
/**
 * How long the public-API snapshot (players / universe / highscore / alliances) is
 * considered current.
 *
 * These files are regenerated by Gameforge on a fixed schedule (hourly to daily), so
 * re-fetching them more often buys nothing and only adds requests - AGENTS.md 6 asks
 * for exactly this: cache and respect the update interval. Before this constant the
 * only guard was a 1-minute in-memory throttle, which meant every page load past that
 * minute re-fetched 12 XML documents (9 of them highscore categories).
 *
 * The key is persisted through `universe.expirations.js`, the same mechanism
 * `parsers/universe.data.js` uses for `serverData.xml`.
 */
const API_SNAPSHOT_KEY = "apiData";
const API_SNAPSHOT_TTL_MS = 12 * 3600 * 1000;
...
export class DataHelper {
  ...
  /**
   * Refreshes the public-API snapshot, at most once per API_SNAPSHOT_TTL_MS.
   *
   * Runs on page load only - never on a timer or a loop (AGENTS.md 1.3 / 4).
   *
   * @param {boolean} [force] bypass the TTL; used by `clearData()`, which has just
   *   thrown the snapshot away and has nothing left to serve from cache.
   * @return {Promise<boolean>} whether the snapshot was actually refetched. `false`
   *   lets the caller skip re-serializing the (large) universe blob for nothing.
   */
  async update(force = false) {
    const logger = getLogger("updateUniverse");

    if (this.loading) return false;
    // A live TTL is only trustworthy while the snapshot it was set for is still there.
    // The expiration key and the `[UNIVERSE]` blob live under separate storage keys, so
    // one can be dropped without the other (a manual reset, an eviction) - without this
    // the extension would sit on an empty universe for the rest of the TTL.
    const hasSnapshot = Boolean(this.players && Object.keys(this.players).length > 0);
    if (!force && hasSnapshot && !(await isUniverseExpired(this.universe, API_SNAPSHOT_KEY))) {
      logger.debug("Ogame's API snapshot still current, last update was: " + this.lastUpdate);
      return false;
    }
    ...
    try {
      ...
      // Only after a successful refresh: a failed fetch must stay expired so the next
      // page load retries instead of serving nothing for 12 hours.
      await setUniverseExpirationTTL(this.universe, API_SNAPSHOT_KEY, API_SNAPSHOT_TTL_MS);
    } catch (err) {
      logger.error(err);
    } finally {
      this.loading = false;
